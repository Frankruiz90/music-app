...
export const Card = ({ tracks, childClick, location = "" }) => {
  ...
  const timeDuration = (duration) => {
    ...
    let total_minuts = parseInt(Math.floor(total_seconds / 60));
  ...
  return (
    <div className="card m-3 bg-dark text-white m-card text-center rounded-1">
      ...
      <div className="card-body position-relative">
        ...
        <p className="card-text">
          <strong>Duration:</strong> {timeDuration(tracks.duration_ms)}
        </p>
        <a
          className="m-card__button position-absolute"
          id={track.id}
          onClick={() => childClick(track.id)}
        >
          {location === "favorites" ? (
            <svg
